...
const FavoriteProvider = ({children}) => {
  ...
  const addToFavorite=(data,findFavoriteItem)=>{
    if(findFavoriteItem){
    var filtered=favoriteItems.filter((item)=>item.id!==findFavoriteItem.id);
  ...
  const removeFromFavorite=(item_id)=>{
    var filtered=favoriteItems.filter((item)=>item.id!==item_id.id)
    setFavoriteItems(filtered);
  }


  var values= { favoriteItems, setFavoriteItems, addToFavorite , removeFromFavorite};
  /**
