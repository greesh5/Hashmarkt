...
const CartProvider = ({children}) => {
  ...
  const addToCart=(data,findCartItem)=>{
    if(findCartItem){
    var filtered=items.filter((item)=>item.id!==findCartItem.id);
    setItems(filtered)}
    else
    setItems(()=>[data,...items]);
  };
  ...
  const removeFromCart=(item_id)=>{
    var filtered=items.filter((item)=>item.id!==item_id.id)
    setItems(filtered);
  }
  const setCartItemsWrapper = (items) => {
    setItems(items);
  };

  var values= { items, setItems:setCartItemsWrapper, addToCart , removeFromCart };
